...
/**
 * IMDEX HubIQ specific helpers
 */
export class HubIQHelpers {
  /**
   * Parse a count from a data card (e.g., "88" from pending data card)
   */
  static parseCardCount(text: string): number {
    const match = text.match(/(\d+)/);
    return match ? parseInt(match[1], 10) : 0;
  }

  /**
   * Parse survey depth value with precision
   */
  static parseSurveyValue(text: string, decimalPlaces = 3): number {
    const num = parseFloat(text);
    return isNaN(num) ? 0 : parseFloat(num.toFixed(decimalPlaces));
  }

  /**
   * Format coordinate with specified precision
   */
  static formatCoordinate(value: number, decimalPlaces = 5): string {
    return value.toFixed(decimalPlaces);
  }

  /**
   * Get current date in the format used by IMDEX HubIQ date pickers
   */
  static getHubIQDateFormatted(date: Date = new Date()): string {
    return formatDate(date);
  }
}
